...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, neunzehnte Fassung nach achtzehn Gutachten**
...
Die übrigen Entitätsstrukturen bauen nach demselben Muster: **jede Entität trägt `angelegtDurch` und `angelegtMit`; jedes Feld, das der Katalog in §5 als Feldpfad nennt, ist ein `Feld<T>` — auch die, die nur eine Anlage setzt und die danach unveränderlich sind (`meldung.stand`, `auftrag.von`, `anhang.dateiname` und ihresgleichen); jedes Merkmal, das §5 als abgeleitet benennt, steht daneben ohne HLC; **und jede Entität, deren Art unter den dreizehn Anlagearten steht (§3.11), trägt `verworfeneAnlagen: VerworfeneAnlage[]` — immer vorhanden, im Regelfall leer.** Ausgenommen sind allein `Meldung` und `Anhang`: Sie gehen nach §3.11 über den Inhaltsschlüssel und legen ihre Verlierer in `verworfeneSchluessel` ab, tragen also **kein** `verworfeneAnlagen`. Ohne diese beiden Sätze wäre `zweiteAnlageVerworfen` an einem Fahrzeug nach einem Schnappschuss nicht mehr rechenbar (P7 fiele), und zwei Umsetzungen unterschieden sich in jedem gesunden Einsatz um so viele Schlüssel, wie er Personen hat — §7.6 der Speicherschicht behält leere Listen (T167).
...
6. **`unbekannt`** hält einen Eintrag je Ereignis einer unbekannten Art oder Version und wächst damit **mit der Zahl der Ereignisse** — im gemischten Betrieb, den §3.9 zum geplanten Normalfall erklärt, ist das kein Randfall. Die Schranke ist die Zahl der Ereignisse, die ein Client nicht kennt; sie ist klein, solange die Fassungen nicht weit auseinanderliegen, und unbeschränkt, wenn doch. Benannt in §8.2, und in der Messung aus B4 mitzuwiegen.
7. **Alles Übrige** ist genau ein Wert je Feld je Entität.
...
> **Jede wachsende Liste des Zustands ist eine Menge:** Zwei Einträge mit identischer kanonischer Serialisierung werden zu einem zusammengezogen. §3.2 legt ihre Ordnung fest, diese Regel ihre Vielfachheit. Es sind **sieben**, und die Aufzählung ist abschließend: `Erstwert.verdraengt`, `verworfeneAnlagen`, `verworfeneSchluessel`, `wartend`, `hinweise`, `unbekannt` und **`einsatz.archivierungen[].zurueckgenommenDurch`**. Die letzte gehört dazu, obwohl sie nicht im selben Atemzug entsteht: Zwei Clients können dieselbe Archivierung gleichzeitig zurücknehmen (§7.2), und dieselbe Rücknahme kann zweimal zugestellt werden. Ohne die Regel stünde sie zweimal, und P2 fiele an der einzigen Stelle, an der der Zustand je Ereignis-Id einen Eintrag führt.
...
**T143** Aufteilung U (0/0/10) → V (0/0/3, HLC 100), danach `EinheitGemeldet(V, 0/0/9)` (HLC 150) ⇒ `angelegtDurch` bleibt die Aufteilung, `V.staerke` ist 0/0/9, `zweiteAnlageVerworfen` steht. **Und die Gegenprobe:** dieselbe Menge mit einem Schnitt zwischen HLC 100 und 150, dazu ein `EinheitVerschoben` bei HLC 120 ⇒ derselbe Zustand wie beim vollen Fold, einschließlich der Hinweise. Eine Umsetzung, die die verdrängte Anlage ihre Felder räumen lässt, muss diesen Teil fallen lassen.
...
**T173 (Fahrzeug in einer Kette ohne Ende):** Fahrzeug in A, `AbschnittAufgeloest(A→B)`, `AbschnittAufgeloest(B→A)` ⇒ `fahrzeug/<id>/wirksamerAbschnittId` ist **A**, der eigene Abschnitt, plus `abschnittAufgeloest` für A; dieselbe Menge mit einer Einheit statt eines Fahrzeugs ⇒ Auffang. Kette A→B→Q mit unbekanntem Q ⇒ ebenfalls A, plus `fremdreferenzUnbekannt`, **nicht** `abschnittUnbekannt`.
