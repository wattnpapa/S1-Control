...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, vierzehnte Fassung nach dreizehn Gutachten**
...
| `ereignisIdDoppeltVergeben` | `id: EreignisId` · `verworfenerInhalt: unknown` |
...
| `entfernungNimmtZugewachsenes` | `entfernt: EreignisId` · `betroffene: { einheitId: Id, staerke: Staerke }[]`, nach `einheitId` sortiert |
...
| `wirkungslosGegenTerminalzustand` | `ereignis: EreignisId` · `grund: Wirkungslosgrund` — geschlossene Liste mit sechs Werten, je einer für die sechs Stellen aus §3.12 |
| `nachArchivierungEingegangen` | `archivierung: EreignisId` · `betroffeneFelder: string[]` (aufsteigend) |
| `undoTrifftFremdenStand` | `undo: EreignisId` · `verdraengt: EreignisId` |
...
**Warum `unknown` an drei Stellen.** `inhalt`, `verworfenerInhalt`, `gesehen` und `verdraengterWert` tragen den Wert eines beliebigen Feldes — ein Stärke-Tripel, einen Text, eine Struktur. Sie gehen als kanonische Serialisierung ihres Wertes in den Hash; einen engeren Typ gibt es nicht, ohne je Feldpfad einen eigenen Hinweis zu bauen. Prüffall T166.
...
| Struktur jeder Hinweisart | §3.8a | T166, T76 |
