...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, dreißigste Fassung nach neunundzwanzig Gutachten**
...
interface VerworfeneAnlage { durch: EreignisId; hlc: Hlc
                             ereignisart: string; inhalt: unknown
                             grund?: string }   // getrennt, weil Rahmenfeld
...
interface EinsatzZustand {
  ...
  angelegtMitGrund?: string                 // §3.11; nur wo die Anlage einen trug
  angelegtMitArt: string                    // §3.11; Ereignisart der geltenden Anlage
  ...
  // abgeleitet:
  id: Id                                      // §7.1, aus der geltenden Anlage
...
interface AbschnittZustand {
  ...
  angelegtMitArt?: string                          // fehlt bei den Systemabschnitten
...
interface Zustand {
  ...
  archivierungen: { [ereignisId: string]: {   // §7.2, unabhängig von `einsatz`
      ...
      zurueckgenommenDurch: { ereignisId: EreignisId
                              erwarteteHlc: string
                              hlc: Hlc }[]   // nach ereignisId sortiert, §7.2
                                             // `erwarteteHlc` ist die Textform der
                                             // erwarteten Archivierung (§7.2), `hlc`
...
7. **`angelegtMitNutzlast`, `angelegtMitGrund` und `angelegtMitArt`** sind genau **eine** Nutzlast, **ein** Freitext und **ein** Artname je Entität und wächst damit mit der Zahl der Entitäten, nicht mit der Zahl der Ereignisse. **Nicht** dieselbe Klasse wie die Felder selbst: Die Nutzlast einer `EinheitAufgeteilt` trägt `uebernommeneFahrzeuge` und `uebernommenePersonen` und ist bei einem Löschzug mit vierzig Personen deutlich größer als die Summe der Felder, die sie setzt. Die Schranke hält, der Posten ist aber in B4 getrennt zu messen.
...
> **Jede mehrwertige Liste des Zustands ist eine Menge:** Zwei Einträge mit identischer kanonischer Serialisierung werden zu einem zusammengezogen. §3.2 legt ihre Ordnung fest, diese Regel ihre Vielfachheit. Es sind **sieben**, und die Aufzählung ist abschließend (`wartend` gehört dazu, obwohl es nach §3.2 Schranke 4 schrumpft — die Mengeneigenschaft hängt nicht am Wachstum): `Erstwert.verdraengt`, `verworfeneAnlagen`, `verworfeneSchluessel`, `wartend`, `hinweise`, `unbekannt` und **`archivierungen[].zurueckgenommenDurch`**. Die letzte gehört dazu, obwohl sie nicht im selben Atemzug entsteht: Zwei Clients können dieselbe Archivierung gleichzeitig zurücknehmen (§7.2), und dieselbe Rücknahme kann zweimal zugestellt werden. Ohne die Regel stünde sie zweimal, und P2 fiele an der einzigen Stelle, an der der Zustand je Ereignis-Id einen Eintrag führt.
...
**`einheitSchluessel` geht ausdrücklich nicht ein,** obwohl er inhaltstragend aussieht. Er ist über `EebMeldungZugeordnet` **änderbar**, und ein Vergleich gegen ein änderbares Zustandsfeld wäre reihenfolgeabhängig: Trifft die zweite Meldung vor der Zuordnung ein, sind die Werte gleich und es entsteht kein Eintrag; trifft sie danach ein, entsteht einer. `verworfeneSchluessel` steht im Zustand und im `zustandsHash` — P1 und P3 fielen über einer vollkommen gesunden Ereignismenge. Die erste Begründung genügt; die Nutzlast der geltenden Anlage steht seit §3.11 sehr wohl im Zustand (`angelegtMitNutzlast`), und **gegen sie** wird auch verglichen — nur eben nicht gegen das änderbare Feld. Prüffall T142.

**Was in `verworfeneSchluessel[].inhalt` steht:** die vollständige `nutzlast` des verworfenen Ereignisses, so wie in `VerworfeneAnlage.inhalt` (§3.8a) — und wie dort stehen `grund`, `ereignisart`, `neu` und `vorher` **nicht** darin, sondern als eigene Felder daneben (§3.2). **Nicht** nur die Felder, die in den Inhaltsvergleich eingehen — die Ablage soll zeigen, was jemand geschrieben hat, nicht, woran der Vergleich sich stieß. **Bei einem Ereignis der Form (a) steht der Wert im Rahmen und nicht in der Nutzlast**; `neu` und `vorher` stehen deshalb als **eigene Felder des Eintrags** daneben, so wie `ereignisart` und `grund`. `inhalt` ist in **jeder** Form allein die Nutzlast — ein Feld, das je nach Form etwas anderes enthielte, wäre genau die Zweideutigkeit, gegen die dieser Absatz geschrieben ist, und `verworfeneSchluessel` steht im `zustandsHash` (T190). Abwesende Felder fehlen, wie überall. **`ereignisart` steht an jedem Eintrag beider Ablagen** — in `verworfeneSchluessel` und in `VerworfeneAnlage`, bei Form (a) wie bei Form (b). Auch bei einer Anlage ist die Art durch die Entität nur eingegrenzt, nicht bestimmt (`EinheitGemeldet` und `EinheitAufgeteilt` legen beide eine Einheit an), und eine Fallunterscheidung nach Form wäre wieder Ratearbeit. Das Feld heißt `ereignisart` und nicht `typ`, weil eine Nutzlast selbst ein Feld `typ` tragen kann (`AbschnittAngelegt.typ` ist der Abschnittstyp); zwei Bedeutungen unter einem Namen wären genau die Ratearbeit, gegen die `art` eingeführt wurde. **„Vorhanden" heißt gesetzt, auch mit leerer Zeichenkette.** `grund` ist im Rahmen `z.string().optional()` und nur bei den neun Arten aus §2.4 auf `min(1)` gezogen; ein `EinheitGemeldet` mit `grund: ""` ist gültig, und der leere String wird abgelegt und serialisiert. Ohne diesen Satz hätten zwei Umsetzungen — „Schlüssel gesetzt" gegen „nichtleer" — verschiedene Bytes. `grund` gehört dazu, weil er bei `AbschnittTypGeaendert` Pflicht ist und die Ablage sonst zeigte, **was** verworfen wurde, nie **warum**. „Ohne die abwesenden Felder" heißt dabei: Trägt das Ereignis kein `vorher` und keinen `grund`, fehlen die Schlüssel; sie werden nicht als `null` geschrieben (§3.2). Die Ereignisart gehört hinein, denn sonst wären `AbschnittUmbenannt(EINGANG, neu: "Sammelraum")` und `AbschnittTypGeaendert(EINGANG, neu: "Sammelraum")` bytegleich, und der Bediener sähe zwar, dass etwas verworfen wurde, aber nicht was. Trägt das verworfene Ereignis die Art `KorrekturVon`, ist `ereignisart` **`"KorrekturVon"`** und `inhalt` seine eigene, vollständige Nutzlast einschließlich `zielTyp` und `korrigiertesEreignisId`. Es wirkt zwar wie eines seines `zielTyp` (§5.9.2), aber die Ablage soll zeigen, was jemand geschrieben hat — und geschrieben wurde eine Berichtigung. **Und `schluessel` ist bei `art: "RESERVIERTE_ID"` die reservierte Entitäts-Id selbst** — `"EINGANG"` oder `"ARCHIV"`, ohne Pfadpräfix; bei `art: "INHALTSSCHLUESSEL"` der Pfad `meldung/<meldungId>` beziehungsweise `anhang/<anhangId>` — die beiden Räume sind getrennt (unten). Das betrifft die ändernden Ereignisse auf reservierte Ids (§5.3.4): Ohne den Zusatz trügen `AbschnittTypGeaendert(EINGANG)` und `AbschnittUmsortiert(EINGANG)` beide nur `{abschnittId: "EINGANG"}`, und der Bediener sähe, dass etwas verworfen wurde, nie was. Das Feld steht im Zustand und damit im Hash; ohne diesen Satz wären zwei Lesarten gedeckt (T53).
...
**`art` unterscheidet die beiden Quellen.** Aus `verworfeneSchluessel` speisen sich zwei Hinweisarten: `inhaltsschluesselWidersprochen` hier und `reservierteIdVerworfen` in §5.3.4. Ohne den Diskriminator müsste ein Client sie an der Form des Schlüssels erraten — `"EINGANG"` gegen Inhalts-Hash —, und zwei Implementierungen rieten verschieden: verschiedene Hinweise, verschiedener `zustandsHash`, roter Ausgang ohne Fold-Fehler. Prüffall T136.
...
Damit das geht, muss **jedes Eingangsdatum eines Hinweises im Zustand stehen**. Das ist der Grund für elf Angaben, die sonst niemand bräuchte: `Beobachtung.wanduhr`, `.fachlicheZeit` und `.zeitklasse` (für `meldezeitUnplausibel`), `Beobachtung.undoOf` (für `undoTrifftFremdenStand`), `angelegtDurch`, `angelegtMitNutzlast`, `angelegtMitGrund` und `angelegtMitArt` je Entität, `zurueckgenommenDurch[].hlc` (für `nachArchivierungEingegangen` an einer Rücknahme, §7.3) sowie `verworfeneAnlagen` je Entität und `verworfeneSchluessel` global (für `zweiteAnlageVerworfen`, `reservierteIdVerworfen`, `inhaltsschluesselWidersprochen`). M0.2 hält diese Angaben im Akkumulator statt im Zustand; das ist die Änderung, die `foldVersion 2` unter anderem trägt.
...
| `reservierteIdVerworfen` | Anlage **oder Änderung** auf `EINGANG` oder `ARCHIV` | §5.3.4 |
...
| `abschnittUnbekannt` | die Einheit zeigt auf einen unbekannten Abschnitt, sie liegt im Eingang | §5.3.3 |
...
| `zweiteAnlageVerworfen` | `verworfen: EreignisId` · `gilt: EreignisId` · **`ereignisart: string`** · `inhalt: unknown` (die Nutzlast aus `VerworfeneAnlage.inhalt`) · **`grund?: string`** (aus `VerworfeneAnlage.grund`), sofern **das Ereignis** einen trug — der `grund` steht dort als **eigenes Feld neben** `inhalt` und nicht darin, weil `inhalt` überall die reine Nutzlast ist und eine zweite Lesart desselben Feldes im `zustandsHash` stünde (T188) — nicht: sofern die Art einen verlangt. `grund` ist nach §2.4 überall erlaubt und nur bei neun Arten Pflicht; maßgeblich ist wie überall das Vorhandensein am Ereignis, nicht die Pflichtigkeit der Art (§3.6). Dieselbe Begründung: Die Ablage soll zeigen, was jemand geschrieben hat, und dazu gehört das Warum |
...
**Dieses Konzept setzt `foldVersion = 3`** (M0.2 hat 1, der Stand vor dem nicht zählenden Eingang 2). Die Erhöhung ist zwingend: §3.2 erweitert den Zustand, §3.8 führt neue Hinweisarten ein, und ein Schnappschuss aus M0.2 trüge weder `zweiter` noch die Eingangsdaten der Hinweise.
...
**Das Fahrzeug folgt derselben Kette, landet aber nie im Eingang** (§5.3.3). Endet sie regulär, gilt das Ziel wie bei der Einheit. Endet sie im Unbekannten oder im Kreis, **behält das Fahrzeug seinen eigenen `abschnittId`** — den Abschnitt, auf den es selbst zeigt, also dasselbe erste Kettenglied, das auch der Hinweis trägt (§5.3.2). Nicht „das letzte auflösbare Glied": In einem Kreis ist jedes Glied auflösbar, und „der Abschnitt, an dem die Verfolgung abbricht" hätte zwei Lesarten. Der eigene `abschnittId` ist der einzige Anker, der nicht davon abhängt, wie weit ein Client die Kette kennt. Den Eingang bekommt das Fahrzeug nie: Der hängt an der Einheit, und ein Fahrzeug hängt an seiner (§5.3.3).

**Die Hinweise dabei:** beim Kreis `abschnittAufgeloest` für das erste Kettenglied, mit dem Feldpfad des Fahrzeugs — wie bei der Einheit. Beim unbekannten Ende **beides**: `abschnittAufgeloest` für das erste Kettenglied und `fremdreferenzUnbekannt` mit der unbekannten Id — dieselben zwei wie bei der Einheit (§5.3.2), nur mit `fremdreferenzUnbekannt` statt `abschnittUnbekannt`: Letzterer sagt nach §3.8 „die Einheit liegt im Eingang", und das trifft hier gerade nicht zu. §5.3.3 nennt für das Fahrzeug denselben Hinweis, wenn es unmittelbar auf einen unbekannten Abschnitt zeigt; die Kette ändert daran nichts. Prüffall T173.

**Die einzige Ausnahme ist der Abschnitt einer Einheit** (§5.3.3): Dort tritt der Eingang an die Stelle des Verweises, damit eine real gemeldete Einheit sichtbar bleibt, statt auf einen Abschnitt zu zeigen, den es nicht gibt. Das ist eine Zusicherung über Sichtbarkeit, keine über Verweise.

### §3.11 Zwei Anlagen derselben Id: die kleinste HLC gilt

Liegen zwei Anlagen derselben Entitäts-Id vor, gilt die mit der **kleinsten** HLC (bei Gleichstand die kleinere Id, §3.5); jede weitere geht in `verworfeneAnlagen` und erzeugt `zweiteAnlageVerworfen` mit ihrer Nutzlast (`inhalt`), ihrer **eigenen** Ereignis-Id (`verworfen`), ihrer Ereignisart (`ereignisart`), ihrem `grund`, sofern sie einen trug, und der Id der geltenden Anlage (`gilt`, im Zustand `angelegtDurch`, §3.2) — die Tabelle in §3.8a ist maßgeblich und abschließend. Die eigene Id ist nicht Zierat: Drei Clients, die dieselbe Einheit inhaltsgleich anlegen, erzeugen sonst zwei bytegleiche Hinweise, und §3.8 zöge sie zu einem zusammen — die Führungsstelle sähe eine Doppelanlage statt zweier. Dasselbe gilt für `reservierteIdVerworfen` (§5.3.4).
...
>
> **Damit auch der Wechsel von `angelegtDurch` ohne Rücknahme auskommt,** hält der Zustand die Nutzlast der **geltenden** Anlage in `angelegtMitNutzlast`, ihren `grund` in `angelegtMitGrund` und ihre **Ereignisart** in `angelegtMitArt` (§3.2). Die Art muss dort stehen, seit `zweiteAnlageVerworfen` und `VerworfeneAnlage` ein `ereignisart` führen (§3.6): Die Entität grenzt die Art nur ein, ohne sie zu bestimmen — `EinheitGemeldet` und `EinheitAufgeteilt` legen beide eine Einheit an, `EtbEintragErfasst` und `EtbEintragBerichtigt` beide einen Tagebucheintrag —, und aus der Nutzlast zu raten verbietet §3.6 ausdrücklich; ein Schema ist nach §3.7 nicht `strict`, eine höhere Nutzlastversion darf zusätzliche Felder mitführen. Ohne `angelegtMitArt` könnte ein Client aus einem Schnappschuss die Art der bisher geltenden Anlage nach ihrer Verdrängung nicht mehr nennen: zwei zulässige Lesarten, zwei `zustandsHash` bei gleichem Versionsvektor, P7 fiele (T193). Der `grund` steht dort und nicht in der Nutzlast, weil er ein Rahmenfeld ist (§2.1) — und er muss dort stehen, seit `zweiteAnlageVerworfen.inhalt` ihn trägt (§3.8a): Sonst könnte ein Client aus einem Schnappschuss ihn nach dem Wechsel der geltenden Anlage nicht mehr nennen, und P7 fiele (T186). Trifft später eine Anlage mit kleinerer HLC ein, wandert die bisherige mitsamt ihrer Nutzlast, ihrem `grund` **und ihrer Art** nach `verworfeneAnlagen`, und die neue nimmt ihren Platz ein — beides sind Umbuchungen innerhalb einer Menge, die nur wächst. Ohne dieses Feld wäre `zweiteAnlageVerworfen` nach einem Schnappschuss nicht mehr zu bilden: Die Felder der Entität sind inzwischen überschrieben, und ein optionales Feld, das die Anlage weggelassen hat, ist von einem gesetzten nicht mehr zu unterscheiden. Es kostet eine Nutzlast je Entität; das ist die Schranke 7 aus §3.2 und in B4 mitzumessen.
>
...
**T1:** Zwei `EinsatzAngelegt` verschiedener HLC, Namen **und `einsatzId`** ⇒ `angelegtDurch` ist die mit der **kleineren** HLC, `einsatz.id` stammt aus ihr, der **Name** der größeren gilt (gewöhnliche Auswahl, §3.11), und es entstehen `zweiteAnlageVerworfen` **und** `ohneVorherWertVerdraengt`. Die beiden Felder folgen also verschiedenen Regeln, und das ist Absicht: Die Kennung ist keine Beobachtung (§2.3). **T2:** `EinsatzBeendet` (HLC 5) und `EinsatzWiedereroeffnet` (HLC 7, `neu = null`) in beiden Permutationen ⇒ `ende` trägt `null` mit Feld-HLC 7, unterscheidbar von „nie gesetzt" — und die Serialisierung enthält `"wert": null` ausdrücklich, nicht bloß die HLC (§3.2).
...
const AbschnittZeichenGesetzt    = z.object({ abschnittId: zId })
...
| `AbschnittZeichenGesetzt` | a | `abschnitt/<id>/taktischesZeichen` · `string \| null` | LWW/Feld | frei |
...
| `EINGANG` | **nein** — der Systemabschnitt aus §5.3.4, kein Teil der Führungsorganisation |
| `ARCHIV` | **nein** — der Systemabschnitt aus §5.3.4 |

Das sind **neun** Werte, dieselben neun, die `ereignis.ts` führt. `EINGANG` ist seit der `foldVersion` 3 dabei: Der systemseitige Eingang trug vorher `EINSATZORT` und zählte damit mit.
...
3. **Ist das Ziel selbst aufgelöst, wird der Kette gefolgt.** Sie endet auf drei Weisen, und jede hat ihre eigenen Hinweise. **Endet sie in einem unbekannten Abschnitt**, landet die Einheit im Eingang, mit `abschnittAufgeloest` für das **erste** Kettenglied (das, auf das die Einheit selbst zeigt, mit dem Ziel, das dessen eigene Auflösung benennt) und `abschnittUnbekannt` mit der unbekannten Id. **Schließt sie einen Kreis** — jeder Abschnitt der Kette existiert —, landet die Einheit ebenfalls im Eingang, aber allein mit `abschnittAufgeloest` für das erste Kettenglied: Es gibt hier keinen unbekannten Abschnitt, und `abschnittUnbekannt` hätte kein Feld zu füllen. **Endet sie regulär**, gilt das Ziel, mit `abschnittAufgeloest` für das erste Glied. Immer das erste Glied, damit der Hinweis nicht davon abhängt, wie weit ein Client die Kette schon kennt. Der Abbruch bei der ersten Wiederholung macht die Verfolgung linear. Prüffall T169.

Alles, was die Kette braucht, steht im Zustand; die Regel gilt damit auch nach einem Schnappschuss.

**T7:** Einheit in A, Auflösung A → B ⇒ wirksam B. **T8:** Verschiebung nach A (9), Auflösung A → B (5) ⇒ wirksam B, `abschnittId` bleibt A, Hinweis. **T9:** A→B, B→C ⇒ C. **T10:** A→B, B→A, beide Abschnitte bekannt ⇒ Eingang, **ein** Hinweis (`abschnittAufgeloest` für das erste Kettenglied); `abschnittUnbekannt` entsteht nicht, es hätte kein Feld zu füllen (§5.3.2, T169). **T11:** Zwei Auflösungen von A mit Zielen B und C ⇒ Ziel der höheren HLC, `vorherPasstNicht`. **T12 (Rebase):** T9, Schnappschuss, dann `AbschnittAufgeloest(C → D)` ⇒ wie voller Fold.

#### §5.3.3 Regel: der Eingangsabschnitt — und was er nicht ist (Auflage 10)

**Zwei Regeln, nicht eine:**

* **Abschnitt unbekannt** — das `AbschnittAngelegt` ist noch unterwegs. Der Zustand ist **vorläufig**: Sobald es eintrifft, steht die Einheit ohne Zutun richtig. Bis dahin Eingang, Hinweis `abschnittUnbekannt`.
* **Abschnitt aufgelöst** — eine Handlung mit **benanntem Ziel**. Die Einheit in den Eingang zu legen wäre eine dauerhafte Verschlechterung.

Beide teilen die Zusicherung: **Eine real gemeldete Einheit verschwindet nie aus dem Zustand, weil ein Abschnitt fehlt.** Sie steht sichtbar im Eingang, und ihre Id ist deshalb reserviert.

**Der Eingang zählt nicht.** Sein Typ ist `EINGANG`, `zaehltInGesamtstaerke` ist falsch, und er erscheint weder im Druck noch in der Führungsorganisation; ein taktisches Zeichen trägt er ebenfalls nicht. Er ist die Warteschlange **vor** der Führungsorganisation, kein Abschnitt darin. Das ist die Umkehrung der früheren Festlegung (`foldVersion` 2 gab ihm `EINSATZORT` und zählte ihn mit) und kostet eine `foldVersion` (§3.9).

**Der Preis, benannt.** Eine Einheit im Eingang fehlt in der Gesamtstärke, solange das `AbschnittAngelegt` unterwegs ist; trifft es ein und ist der Typ zählend, springt die Gesamtstärke nach oben. Die Gegenrichtung — mitzählen und beim Eintreffen eines `ANGEFORDERT` wieder abziehen — wäre ebenso ein Sprung, nur einer, der die Führungsorganisation mit Kräften füllt, die dort niemand geführt hat. Die Einheit ist nicht verloren: Sie steht im Eingang, mit `abschnittUnbekannt`. In §8.2 geführt. Ein Abschnitt mit **unbekanntem** Typ zählt dagegen weiter wie `EINSATZORT` (§3.7) — er ist gemeldet, nur sein Typ ist es nicht.

**Fahrzeuge gehen nicht in den Eingang.** Ein Fahrzeug hat keine Stärke; die Zusicherung greift nicht. Bei **unmittelbar** unbekanntem Abschnitt bleibt `abschnittId` gefaltet stehen, `wirksamerAbschnittId` ist **abwesend** (das Fahrzeug hängt an seiner Einheit), und es entsteht `fremdreferenzUnbekannt` mit dieser Id. Bei **aufgelöstem** Abschnitt wird derselben Kette gefolgt wie bei der Einheit, aber mit einem anderen Ende: Endet sie regulär, gilt das Ziel; endet sie im Kreis oder im Unbekannten, behält das Fahrzeug **seinen eigenen `abschnittId`** — nicht den Eingang und nicht „abwesend“, denn der Abschnitt, auf den es zeigt, existiert ja (§3.10). Die Hinweise dabei stehen in §3.10. Damit ist P5 sauber getrennt: Es spricht von Einheiten.

**T13:** `EinheitGemeldet` in Abschnitt Q ohne dessen Anlage ⇒ Eingang, Hinweis. **T14:** Plus `AbschnittAngelegt(Q)` ⇒ Einheit in Q, kein Hinweis, jede Permutation. **T15:** `FahrzeugVerschoben` in unbekannten Abschnitt ⇒ kein Eingang, `fremdreferenzUnbekannt`. **T16:** Abschnitt mit unbekanntem Typ ⇒ `zaehltInGesamtstaerke` wahr, `unbekannterWert`. **T13a:** Einheit im Eingang ⇒ `zaehlt` falsch; nach `AbschnittAngelegt(Q)` mit zählendem Typ ⇒ `zaehlt` wahr.
...
EINGANG = { id: "EINGANG", systemAbschnitt: true,
...
`name` des Archivs folgt der Excel (`Stärke!B431` „Kopier Bereich für ‚Einsatz beendet'"); `reihenfolge` setzt es ans Ende jeder Sortierung. Beide Ids sind **reserviert**: Eine Anlage darauf wird verworfen und erzeugt `reservierteIdVerworfen`. Ohne die Reservierung könnte eine Anlage dem Eingang einen zählenden Typ geben, und Kräfte, die noch niemand geführt hat, stünden in der Gesamtstärke. **Auch jedes ändernde Ereignis auf diese beiden Ids ist wirkungslos** — `AbschnittTypGeaendert`, `AbschnittUmbenannt`, `AbschnittUmgehaengt`, `AbschnittUmsortiert`, `AbschnittBemerkungGesetzt`, `AbschnittZeichenGesetzt`, `AbschnittAufgeloest` und `AbschnittWiederhergestellt` — **alle acht**, die §5.3 kennt. Ohne diesen Satz genügte ein `AbschnittTypGeaendert(EINGANG, "EINSATZORT")`, um die Stärke jeder dort liegenden Einheit in die Gesamtstärke zu holen, und ein `AbschnittAufgeloest(EINGANG)` machte P5 unerfüllbar: Die Kette aus §5.3.2 endete wieder im Eingang, und der wäre aufgelöst. Das Ereignis wird gefaltet, es wirkt nicht, und es geht **unter demselben Diskriminator `art: „RESERVIERTE_ID“`** in `verworfeneSchluessel`; der Hinweis `reservierteIdVerworfen` deckt beide Fälle ab, damit es nicht still verpufft. Ablage und Hinweis sind dabei nach §3.2 auf `KAPPUNG_MAX` gekappt: Ein Client, der die Reservierung nicht kennt, kann sie beliebig oft treffen, und der Zustand darf mit der Zahl der Ereignisse nicht wachsen (§3.1). Sein `feldpfad` ist **immer** der Pfad der Entität (`abschnitt/EINGANG` beziehungsweise `abschnitt/ARCHIV`) — bei den acht ändernden Arten wie bei der verworfenen **Anlage**, ohne Fallunterscheidung (§3.6), und auch bei einem ändernden Ereignis, das einen Feldpfad benennt: gesetzt wurde ja keines. Welche Ereignisart verworfen wurde, steht im `ereignisart` des Eintrags in `verworfeneSchluessel` (§3.6) und **nicht** im Hinweis; und `hinweise` ist nach der kanonischen Serialisierung geordnet (§3.8), sodass eine zweite Lesart auch die Listenreihenfolge verschöbe (T177).
...
**T17:** `AbschnittAngelegt` auf `EINGANG` bzw. `ARCHIV` ⇒ verworfen, Hinweis, Werte unverändert. **T18:** Zwei Clients ohne gemeinsame Ereignisse haben für beide Systemabschnitte denselben Teilhash.
...
Die zweite Archivierung erzeugt keine zweite Barriere, wohl aber `wirkungslosGegenTerminalzustand` (§3.12) — solange sie weder die maßgebliche noch zurückgenommen ist (§3.8) — sie ist nicht falsch, nur später, und der Bediener soll sehen, dass sein Klick nichts geändert hat.
...
**Fünf Zustandsteile ohne Entität, und sie werden verschieden behandelt.** `wartend` und `schichtplan` sind erfasst; `unbekannt` und die Einträge in `verworfeneSchluessel` mit `art: „RESERVIERTE_ID“` nicht. Die Einträge mit `art: „INHALTSSCHLUESSEL“` sind **keine Ausnahme** und brauchen auch keine: Ihre Anlage belegt ihre Feldpfade nach der gewöhnlichen Auswahl (§3.11), der Hinweis entsteht also über die Entität `meldung/<id>` beziehungsweise `anhang/<id>` wie bei jeder anderen (T194). **`archivierungen` ist teilweise erfasst:** Eine zweite Archivierung trägt `ZWEITE_ARCHIVIERUNG` (§3.8); eine **Rücknahme** trägt `nachArchivierungEingegangen`, wenn **drei** Bedingungen zugleich gelten:

> **(a)** Es gibt (noch) eine maßgebliche Archivierung. **(b)** Die Rücknahme hat **gewirkt**: Ihre `erwarteteHlc` passt zu der Archivierung, die sie benennt (§7.2) — sie ist also weder ein wartender Grabstein noch eine, die ins Leere geht. **(c)** Ihre **eigene** HLC ist größer als die der maßgeblichen Archivierung.
>
> Der Hinweis steht dann **einmal je Feldpfad** — wie jeder `nachArchivierungEingegangen` — mit `feldpfad: "archivierungen/<benannte id>"`, `archivierung:` der Id der **maßgeblichen** Archivierung (die Bedeutung des Feldes ist überall „nach welcher Barriere") und `betroffeneFelder:` den Restpfaden **aller** wirksamen Rücknahmen dieses Eintrags, die (c) erfüllen, aufsteigend: `["zurueckgenommenDurch/<Id>", …]`. Zwei Clients, die dieselbe Archivierung zurücknehmen, erzeugen also einen Hinweis mit zwei Einträgen und nicht zwei Hinweise — dieselbe Form wie bei einer Entität mit zwei geänderten Feldern.

Bedingung (b) hält die Regel mit §3.12 und §8.2 zusammen, und sie prüft die **Wirkung**, nicht bloß das Vorliegen: Eine Rücknahme, die eine Archivierung benennt, die es nicht gibt, erzeugt keinen Hinweis — sie ist von einem wartenden Grabstein nicht zu unterscheiden; und eine mit unpassender `erwarteteHlc` erzeugt bereits `wirkungslosGegenTerminalzustand` an demselben Feldpfad (§3.12), sodass zwei Hinweise für eine folgenlose Handlung entstünden. (a) ist keine Einschränkung, sondern eine Selbstverständlichkeit: Nimmt die Rücknahme die **einzige** Archivierung zurück, ist der Einsatz offen, es gibt keine Barriere, und nach §7.4 verschwinden die Hinweise ohnehin. Weil (b) gilt und die maßgebliche nach (a) weiterhin gilt, hat die Rücknahme zwangsläufig eine **andere** getroffen — der Fall greift also genau dort, wo eine zweite Archivierung die Barriere aufrechterhält und ein Bedienschritt mit Pflicht-`grund` sonst spurlos bliebe.

**Die Schwelle verschiebt sich, wenn die maßgebliche Archivierung wechselt.** Wird sie später zurückgenommen, rückt eine jüngere nach, und eine Rücknahme, die bisher „danach" lag, liegt nun „davor" — ihr Hinweis fällt weg. Das ist eine reine Funktion der Ereignismenge und bricht keine Eigenschaft; es ist auch die einzige Lesart, die zur Bedeutung des Hinweises passt („nach der Barriere, die **gilt**"). §8.2 führt es als Nicht-Zusicherung. Dafür führt `zurueckgenommenDurch` seit der achtundzwanzigsten Fassung die eigene `hlc` der Rücknahme mit (T189). Der Trennstrich ist, ob dort **gemeldete Fachdaten** liegen: In `wartend` steht eine Stärke, die jemand gemeldet hat, und sie kann nach der Archivierung eingegangen sein; in `unbekannt` steht ein Ereignis, das dieser Client nicht deuten kann, und in `verworfeneSchluessel` mit `art: „RESERVIERTE_ID“` eines, das nicht gewirkt hat. **Für `wartend` gilt deshalb: Trägt eine wartende Beobachtung eine HLC größer als die der maßgeblichen Archivierung, entsteht `nachArchivierungEingegangen` mit dem Entitätspfad, unter dem sie wartet** (`einheit/E7`, `dienstposten/D1`), und mit den `feld`-Werten der zu jungen Beobachtungen in `betroffeneFelder` (`staerke`, `schichtplan/<datum>`). Beides ist aus `wartend` unmittelbar ablesbar.

**Der Hinweis kann dabei wandern, und das ist richtig so.** Trifft die fehlende Anlage nach, steht dasselbe Datum an der Entität oder — bei der Schichtplanzelle — unter `schichtplan/<dienstpostenId>`, und der Hinweis steht dort. Er **verschwindet** nicht, er wechselt den Pfad, weil das Datum den Ort wechselt — und bei der Schichtplanzelle mit dem Pfad auch seinen Inhalt: aus `dienstposten/D1` mit `["schichtplan/<datum>"]` wird `schichtplan/D1` mit `["<datum>"]`, weil `betroffeneFelder` immer der Restpfad unter dem Feldpfad ist. Die sechsundzwanzigste Fassung behauptete hier mehr („derselbe Pfad, den die Entität tragen würde") und stimmte für die Schichtplanzelle nicht, weil deren Ziel eine andere Wurzel ist (§3.10). Ausgeschlossen ist allein, dass er ganz ausbleibt (T185). Für die beiden anderen führt §8.2 die Lücke als Nicht-Zusicherung.
...
**Bei den beiden Erstwert-Feldern zählt auch `verdraengt`.** Sonst flackerte der Hinweis: Eine Zusammenführung mit HLC 9 nach einer Archivierung mit HLC 5 stünde in `wartend` als zu jung, verlöre aber nach dem Eintreffen der Anlage gegen den Gewinner mit der **kleineren** HLC und verschwände aus dem Hinweis — obwohl sie unverändert im Zustand steht. Geprüft wird deshalb die höchste HLC **aller** gehaltenen Beobachtungen eines Feldes, nicht nur die des Gewinners (T187). Ein Feld, bei dem allein eine **verdrängte** Beobachtung zu jung ist, steht damit in `betroffeneFelder` — es hat sich etwas nach der Archivierung getan, auch wenn der geltende Wert älter ist. Bei einem `Feld<T>` ist das immer der Gewinner; bei einem `Erstwert<T>` der Gewinner oder, wenn die Liste nicht leer ist, ihr jüngster Eintrag.

**Der Hinweis hängt sonst an der Entität, nicht am Ereignis und nicht am einzelnen Feld.** Für jede Entität, von deren Feldern mindestens eines **eine gehaltene Beobachtung** mit einer HLC größer als die der maßgeblichen Archivierung trägt — Gewinner, `zweiter` oder ein Eintrag in `verdraengt`; bei einem `Feld<T>` ist das immer der Gewinner, bei einem `Erstwert<T>` kann es ein verdrängter sein (oben), entsteht **ein** `nachArchivierungEingegangen` mit dem Entitätspfad und der Liste der betroffenen Felder — als **Restpfad unter diesem Pfad**, nicht als blosser Feldname: `kosten/vdaProTag`, nicht `vdaProTag`, und `logistik/vegetarisch`, nicht `vegetarisch`. Der Katalog kennt zweistufige Feldpfade (§2.3, §5.4), und die kurze Form wäre zwischen zwei Ebenen mehrdeutig. Das ist die einzige Form, die nach §3.1 trägt — der Zustand kennt Felder, nicht den Ereignisstrom — und sie hält die Zahl der Hinweise bei der Zahl der geänderten Entitäten statt bei der ihrer Felder. Bei einer Archivierung mit versehentlich kleiner HLC wären das sonst sechsstellig viele Hinweise im Hash.
...
**T68:** Zwei `EinsatzArchiviert` (5 und 9) ⇒ maßgeblich ist 5, für die zweite ein Wirkungslos-Hinweis. **T69:** `StatusGesetzt` mit HLC 7 dazu ⇒ gefaltet, wirkt, ein `nachArchivierungEingegangen` an der Einheit mit dem Feldnamen. **T70:** Rücknahme der Archivierung mit HLC 5, eigene HLC der Rücknahme 4 ⇒ maßgeblich ist 9, die Einheit aus HLC 7 verliert ihren Hinweis, und die Rücknahme selbst trägt keinen (4 < 9, Bedingung (c) aus §7.3). Mit eigener HLC 30 trägt sie einen — der Prüffall nennt die Zahl, weil sie das Ergebnis bestimmt. **T71 (Grabstein):** Die Rücknahme trifft **vor** ihrer Archivierung ein ⇒ derselbe Zustand wie in umgekehrter Reihenfolge. **T72 (Monotonie):** Archivierung HLC 9, Rücknahme HLC 5 ⇒ der Einsatz ist offen. **T73:** Beide Archivierungen zurückgenommen ⇒ Einsatz offen, keine Entität trägt den Hinweis. **T74:** Alle vorstehenden Fälle in jeder Permutation mit demselben Ergebnis.
...
**Die Gesamtstärke, solange eine Einheit im Eingang liegt** oder ein Abschnittstyp unbekannt ist. Sie kann vorübergehend zu niedrig sein, weil der Eingang nicht zählt (§5.3.3), und bei unbekanntem Typ zu hoch (§3.7). Zugesichert ist, dass keine gemeldete Einheit verschwindet — nicht, dass die Summe in jedem Zwischenstand stimmt. Was im Eingang liegt, zeigt der Hinweis `abschnittUnbekannt`.
...
**Der Hinweis auf nachträgliche Änderungen deckt zwei Zustandsteile nicht ab.** Ein Ereignis, das dieser Client nicht deuten kann (`unbekannt`), und eines, das auf eine reservierte Id zielt (`verworfeneSchluessel`), erzeugen kein `nachArchivierungEingegangen`, auch wenn ihre HLC größer ist als die der maßgeblichen Archivierung (§7.3). Beide tragen keine gemeldeten Fachdaten; die Zusicherung gilt für Felder von Entitäten, für Schichtplanzellen, für wartende Beobachtungen und — in dem in §7.3 benannten Umfang — für die Archivierungen. **Und sie ist dort an die geltende Barriere gebunden:** Wechselt die maßgebliche Archivierung, weil die bisherige zurückgenommen wird, kann der Hinweis auf eine frühere Rücknahme wegfallen, obwohl deren Eintrag unverändert im Zustand steht (§7.3). **Dieselbe Bindung lässt einen Fall von vornherein durchfallen:** Eine Rücknahme, deren eigene HLC zwar größer ist als die der Archivierung, die sie **getroffen** hat, aber kleiner als die der Archivierung, die **danach** maßgeblich ist, erzeugt nach Bedingung (c) **keinen** Hinweis — obwohl der Bediener hinter der damals geltenden Barriere mit Pflicht-`grund` gehandelt hat und der Einsatz archiviert bleibt. Beispiel: Archivierungen mit HLC 5 und 100, Rücknahme der ersten mit eigener HLC 50. Ein `wirkungslosGegenTerminalzustand` entsteht dabei ebenfalls nicht, denn die Rücknahme hat gewirkt. Der Bedienschritt ist im Ereignisstrom sichtbar, im Zustand nicht. Der Fall gegen den Stand **vor** der eigenen Wirkung zu messen verbietet §3.1; deshalb steht er hier und nicht in §7.3 (T195).
...
**Was jenseits der Kappung liegt, zeigt der Zustand nicht.** Ab `KAPPUNG_MAX` gleichartigen Einträgen erscheint ein weiteres unbekanntes Ereignis und ein weiteres Ereignis auf eine reservierte Id nicht mehr im Lagebild — `wartend` ist davon nicht betroffen und wird nach §3.2 Schranke 4 ausdrücklich nicht gekappt (T180) — die Akte behält alles, die Speicherschicht spiegelt alles weiter, und die Zusicherungen über Stärkezahlen sind nicht betroffen. **Die Auswahl ist zudem inhaltlich blind:** Sie nimmt die kleinsten Ids, nicht die schwerwiegendsten Fälle. Fünfzig harmlose Umsortierungen des Eingangs können ein einzelnes `AbschnittTypGeaendert(EINGANG)` verdrängen; wirkungslos bleibt es trotzdem (§5.3.4), sichtbar nicht.
...
| 10 · Zyklusregel; relative Stärkeänderung; **Eingangsregel für aufgelöste Abschnitte** | §5.3.1, §5.4.2, §5.3.3 | **Erfüllt, aber mit einer begründeten Umkehrung:** §5.3.3 gibt dem aufgelösten Abschnitt **nicht** den Eingang, sondern das benannte Ziel der Auflösung; der Eingang bleibt dem *unbekannten* Abschnitt. Die Auflage nennt den umgekehrten Fall. Als Auslegung 14 in §10 geführt |
...
| S11 | Vererbt ein Abschnitt seine Zählbarkeit an Unterabschnitte? | nein; jeder Abschnitt zählt nach seinem eigenen `typ` | §3.2 | **entschieden am 2026-09-10 (Johannes)**; beantwortet die offene Frage 3 aus ZDM §2.4 Nr. 6 gegen deren Vorschlag |

### Befunde für Johannes — nicht durch dieses Dokument geändert

**B1 — `schemaVersion` ist nicht die Version des Rahmens.** KONZEPT-SPEICHER.md §2.4 beschreibt sie so; die DoD von M1.2 und ZDM §4.1 meinen die Nutzlast dieser Art. Dieses Konzept liest sie als Nutzlastversion (§4.1); der Rahmen ist über `manifest.json` versioniert, und die Speicherschicht reicht das Feld ohnehin nur durch. **Vorschlag:** den Satz in §2.4 ändern. **Erledigt am 2026-09-10:** §2.4 dort ist geändert und trägt den Verweis auf diesen Befund.

**B2 — Der Undo-Stapel in §4.4 ist zu eng.** „ein **eigenes** Ereignis mit passendem `undoOf`" → „ein Ereignis". §4.4 verweist die Semantik ausdrücklich hierher; die Änderung bleibt in seiner Absicht. Begründung in §6 U3. **Erledigt am 2026-09-10:** §4.4 dort ist geändert.

**B3 — ZDM §4.1 Regel 5 widerspricht KONZEPT-SPEICHER.md §5.7.** §7.3 entscheidet für die Speicherfassung. **Vorschlag:** Regel 5 streichen oder mit Verweis versehen. **Erledigt am 2026-09-10:** Regel 5 ist gestrichen und trägt den Verweis auf §7.3.

**B4 — Der Zustand wächst gegenüber M0.2 deutlich.** `zweiter` je Feld, `wanduhr` und `fachlicheZeit` je Beobachtung, die verworfenen Anlagen, `angelegtMitNutzlast` und `angelegtMitGrund` je Entität, die abgeleiteten Felder — grob geschätzt Faktor drei bis vier je Schnappschuss. KONZEPT-SPEICHER.md §7.5 kalibriert die Schnappschuss-Auslöser gegen die Erstlaufzeit, nicht gegen die Dateigröße. **Kein Handlungsbedarf vor M0.5**, aber die Messung dort sollte die Schnappschussgröße mitnehmen.

**B6 — `grund` ist im Rahmen, KONZEPT-SPEICHER.md §2.4 kennt es nicht.** §2.1 dieses Dokuments und `ereignis.ts` führen `grund` als Rahmenfeld; §2.4 der Speicherschicht zählt die Rahmenfelder auf und nennt es nicht („alles Weitere ist für sie undurchsichtige Nutzlast"). §3.6 stützt sich seit der fünfundzwanzigsten Fassung darauf, indem es `grund` **neben** `nutzlast` in die Ablage schreibt. Für die Speicherschicht ist das folgenlos — sie reicht den Rahmen durch —, aber die Aufzählung dort ist unvollständig. **Vorschlag:** `grund` in §2.4 nachtragen. Blockiert M1.3 nicht. **Erledigt am 2026-09-10:** `grund` steht in der Tabelle von §2.4 der Speicherschicht.
...
14. **Der Eingang gilt nicht für den aufgelösten Abschnitt** (§5.3.3). Auflage 10 nennt ihn ausdrücklich für diesen Fall; die Umkehrung ist begründet, weicht aber vom Wortlaut ab.
...
| ~~Offene Frage 3 aus ZDM §2.4 Nr. 6: Vererbung der Zählbarkeit~~ | **entschieden am 2026-09-10 (Johannes): nicht vererben.** S11 gilt endgültig; ZDM §2.4 Nr. 6 ist damit gegen seinen eigenen Vorschlag beantwortet |
...
| Zweite Anlage: kleinste HLC für `angelegtDurch`, Werte in der gewöhnlichen Auswahl | §3.11 | T1, T90, T100, T143, T186, T188 |
...
| `art` in `verworfeneSchluessel`; `inhalt` ist die reine Nutzlast | §3.6, §5.3.4 | T136, T190 |
...
| Eingang nur für den unbekannten Abschnitt | §5.3.3 | T13, T14, T16 |
| Fahrzeuge gehen nicht in den Eingang, Kette ohne Ende | §5.3.3, §3.10 | T15, T173 |
...
| Maßgebliche Archivierung, Grabstein, Monotonie, Wurzel | §7.2 | T68, T71, T72, T178, T184, T189 |
| Ereignis nach der Archivierung wirkt; Hinweis je Entität, je Schichtplan, je wartender Beobachtung und je Archivierungseintrag | §7.3 | T69, T181, T185, T187, T189, T191, T192 |
...
**T136** Je ein Eintrag aus widersprochenem Inhaltsschlüssel und aus einer Anlage auf `EINGANG` ⇒ zwei Einträge in `verworfeneSchluessel` mit verschiedenem `art`, und zwei Hinweisarten, ohne dass die Form des Schlüssels ausgewertet wird.
...
**T191 (zwei Rücknahmen derselben Archivierung):** Archivierung HLC 5 (maßgeblich), zweite HLC 9, zwei Rücknahmen der zweiten mit eigenen HLC 20 und 21 ⇒ **ein** `nachArchivierungEingegangen` an `archivierungen/<id der zweiten>` mit beiden Restpfaden in `betroffeneFelder`, aufsteigend. Nicht zwei Hinweise.
**T193 (`angelegtMitArt` über einen Schnitt):** `EtbEintragErfasst(etbId X)` `A:1` HLC 1000, Schnappschuss, danach `EtbEintragBerichtigt(etbId X, berichtigtEintragId W)` `B:1` HLC 500 ⇒ die zweite gilt, `zweiteAnlageVerworfen` trägt `ereignisart: "EtbEintragErfasst"` — beim vollen Fold und beim Client aus dem Schnappschuss gleich. Dieselbe Probe mit `EinheitGemeldet(E1)` HLC 900 gegen `EinheitAufgeteilt(neueEinheitId E1)` HLC 400: Die Entität grenzt die Art nur ein, aus `angelegtMitNutzlast` ist sie nach §3.7 nicht zu erschließen. Ohne `angelegtMitArt` fällt P7.

**T194 (Inhaltsschlüssel nach der Archivierung):** `EinsatzArchiviert` HLC 5, `EebMeldungEmpfangen(meldungId M, empfangenAm 07:12)` HLC 3, derselbe Bogen ein zweites Mal mit `empfangenAm 21:40` und HLC 40 ⇒ der zweite Eingang steht in `verworfeneSchluessel` mit `art: "INHALTSSCHLUESSEL"`, setzt `meldung/M/empfangenAm` auf 21:40 und erzeugt `nachArchivierungEingegangen` an `meldung/M` mit `empfangenAm` in `betroffeneFelder`. Die Ausnahme aus §7.3 gilt nur für `art: "RESERVIERTE_ID"`.

**T195 (Rücknahme zwischen zwei Barrieren):** `EinsatzArchiviert` `A:1` HLC 5, `EinsatzArchiviert` `B:1` HLC 100, `ArchivierungZurueckgenommen(A:1)` mit eigener HLC 50 ⇒ `gilt(A:1)` ist falsch, maßgeblich ist `B:1`, und es entsteht **weder** `nachArchivierungEingegangen` (Bedingung (c): 50 < 100) **noch** `wirkungslosGegenTerminalzustand` (die Rücknahme hat gewirkt). Das ist die Lücke aus §8.2; eine Umsetzung, die hier einen Hinweis erzeugt, weicht ab.

**T192 (unwirksame Rücknahme):** Archivierung `A:1` HLC 5 (maßgeblich); Rücknahme, die **`A:1` benennt**, mit unpassender `erwarteteHlc` und eigener HLC 20 ⇒ **nur** `wirkungslosGegenTerminalzustand` mit `RUECKNAHME_OHNE_PASSENDE_ARCHIVIERUNG`, **kein** `nachArchivierungEingegangen`; sonst trüge eine folgenlose Handlung zwei Hinweise an demselben Feldpfad.
**T190 (`inhalt` ist überall die reine Nutzlast):** Je ein verworfener Eintrag aus einer Anlage (`EebMeldungEmpfangen`), aus einem ändernden Ereignis auf eine reservierte Id (`AbschnittTypGeaendert(EINGANG)` mit `grund`) und aus einem `KorrekturVon` ⇒ alle drei tragen `inhalt` als reine Nutzlast, dazu `ereignisart`, und die beiden letzten `neu` und `grund` als eigene Felder, dazu `vorher` nur dort, wo das Ereignis eines trug. Ein Systemabschnitt ist dabei **kein** Sonderfall: §5.3.4 setzt `EINGANG.typ`, der Bediener sieht den Wert und trägt ihn nach Auflage 6 als `vorher` ein. Kein Eintrag trägt ein Feld, das §3.2 nicht nennt.
**T188 (`grund` der verdrängten Anlage):** Zwei `EtbEintragBerichtigt` derselben Id mit verschiedenem `grund` ⇒ `zweiteAnlageVerworfen` trägt `inhalt` (nur die Nutzlast) **und** `grund` als eigenes Feld; `VerworfeneAnlage` hält beides getrennt, und die Serialisierung ist in jeder Reihenfolge dieselbe.
**T189 (Rücknahme nach der Archivierung):** Archivierung `A:1` HLC 5 (maßgeblich), zweite `B:1` HLC 9, Rücknahme von `B:1` mit eigener HLC 20 ⇒ `ZWEITE_ARCHIVIERUNG` fällt weg, dafür steht `nachArchivierungEingegangen` an `archivierungen/B:1`. Ohne die mitgeführte eigene HLC der Rücknahme wäre der Bedienschritt mit Pflicht-`grund` unsichtbar.
**T186 (`grund` der geltenden Anlage über einen Schnitt):** `EtbEintragBerichtigt(etbId X, grund "Zeitpunkt falsch")` HLC 9, Schnappschuss, danach `EtbEintragBerichtigt(etbId X, grund "Text falsch")` HLC 3 ⇒ `zweiteAnlageVerworfen` trägt den `grund` der verdrängten Anlage, und zwar in beiden Wegen gleich. Ohne `angelegtMitGrund` könnte der Client aus dem Schnappschuss ihn nicht nennen.
**T187 (Erstwert nach der Archivierung):** Archivierung HLC 5; zwei `EinheitZusammengefuehrt` derselben Quelle mit HLC 3 und 9; die Anlage der Quelle trifft erst danach ein ⇒ `nachArchivierungEingegangen` steht **vor und nach** dem Eintreffen der Anlage, weil auch die verdrängte Beobachtung mit HLC 9 zählt.
...
**T173 (Fahrzeug in einer Kette ohne Ende):** Fahrzeug in A, `AbschnittAufgeloest(A→B)`, `AbschnittAufgeloest(B→A)` ⇒ `fahrzeug/<id>/wirksamerAbschnittId` ist **A**, der eigene Abschnitt, plus `abschnittAufgeloest` für A; dieselbe Menge mit einer Einheit statt eines Fahrzeugs ⇒ Eingang. Kette A→B→Q mit unbekanntem Q ⇒ ebenfalls A, plus **zwei** Hinweise: `abschnittAufgeloest` für A und `fremdreferenzUnbekannt` mit Q, **nicht** `abschnittUnbekannt`.
...
**T167 (`verworfeneAnlagen` an jeder Entität):** Zwei `FahrzeugAngelegt` derselben Id mit verschiedenem Kennzeichen ⇒ `zweiteAnlageVerworfen` am Fahrzeug **und** `ohneVorherWertVerdraengt` an `fahrzeug/<id>/kennzeichen`, beide auch nach einem Schnappschuss. Ein Einsatz **ohne** jede Doppelanlage ⇒ jede Entität der elf Entitätsarten unter den dreizehn Anlagearten trägt `verworfeneAnlagen: []` in der Serialisierung; `Meldung` und `Anhang` — die beiden übrigen der dreizehn — tragen das Feld **nicht** (§3.11).
...
**T169 (aufgelöste Kette):** Kette in einen unbekannten Abschnitt ⇒ Eingang, `abschnittAufgeloest` für das **erste** Kettenglied und `abschnittUnbekannt`. Kette in einen Kreis, alle Abschnitte bekannt ⇒ Eingang, **nur** `abschnittAufgeloest` für das erste Glied. Reguläres Ende ⇒ das Ziel gilt, `abschnittAufgeloest` für das erste Glied. In allen drei Fällen dasselbe Kettenglied, unabhängig davon, wie viel der Kette dieser Client kennt.
