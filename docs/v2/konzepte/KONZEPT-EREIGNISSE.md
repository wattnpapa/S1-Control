...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, neunundzwanzigste Fassung nach achtundzwanzig Gutachten**
...
**Fünf Zustandsteile ohne Entität, und sie werden verschieden behandelt.** `wartend` und `schichtplan` sind erfasst; `unbekannt` und die Einträge in `verworfeneSchluessel` nicht. **`archivierungen` ist teilweise erfasst:** Eine zweite Archivierung trägt `ZWEITE_ARCHIVIERUNG` (§3.8); eine **Rücknahme** trägt `nachArchivierungEingegangen`, wenn **drei** Bedingungen zugleich gelten: Es gibt (noch) eine maßgebliche Archivierung, die von der Rücknahme **benannte** Archivierung liegt vor (ihr Eintrag trägt eine `hlc`), und die **eigene** HLC der Rücknahme ist größer als die der maßgeblichen. Dann steht der Hinweis mit

> `feldpfad: "archivierungen/<benannte id>"`, `archivierung:` die Id der **maßgeblichen** Archivierung (wie überall, die Bedeutung des Feldes ist „nach welcher Barriere"), `betroffeneFelder: ["zurueckgenommenDurch/<Id der Rücknahme>"]` — der Restpfad einschließlich des Listeneintrags, damit zwei Rücknahmen derselben Archivierung nicht zu einem Hinweis zusammenfallen (§3.8).

Die zweite Bedingung hält die Regel mit §3.12 und §8.2 zusammen: Eine Rücknahme, die eine Archivierung benennt, die es nicht gibt, erzeugt **keinen** Hinweis — sie ist von einem Grabstein, der auf eine später eintreffende Archivierung wartet, im Zustand nicht zu unterscheiden. Die erste ist keine Einschränkung, sondern eine Selbstverständlichkeit: Nimmt die Rücknahme die **einzige** Archivierung zurück, ist der Einsatz offen, es gibt keine Barriere, und nach §7.4 verschwinden die Hinweise ohnehin. Der neue Fall greift also genau dort, wo eine zweite Archivierung die Barriere aufrechterhält und ein Bedienschritt mit Pflicht-`grund` sonst spurlos bliebe. Dafür führt `zurueckgenommenDurch` seit der achtundzwanzigsten Fassung die eigene `hlc` der Rücknahme mit (T189). Der Trennstrich ist, ob dort **gemeldete Fachdaten** liegen: In `wartend` steht eine Stärke, die jemand gemeldet hat, und sie kann nach der Archivierung eingegangen sein; in `unbekannt` steht ein Ereignis, das dieser Client nicht deuten kann, und in `verworfeneSchluessel` eines, das nicht gewirkt hat. **Für `wartend` gilt deshalb: Trägt eine wartende Beobachtung eine HLC größer als die der maßgeblichen Archivierung, entsteht `nachArchivierungEingegangen` mit dem Entitätspfad, unter dem sie wartet** (`einheit/E7`, `dienstposten/D1`), und mit den `feld`-Werten der zu jungen Beobachtungen in `betroffeneFelder` (`staerke`, `schichtplan/<datum>`). Beides ist aus `wartend` unmittelbar ablesbar.
...
| Ereignis nach der Archivierung wirkt; Hinweis je Entität, je Schichtplan und je wartender Beobachtung | §7.3 | T69, T181, T185, T187 |
...
**T190 (`inhalt` ist überall die reine Nutzlast):** Je ein verworfener Eintrag aus einer Anlage (`EebMeldungEmpfangen`), aus einem ändernden Ereignis auf eine reservierte Id (`AbschnittTypGeaendert(AUFFANG)` mit `grund`) und aus einem `KorrekturVon` ⇒ alle drei tragen `inhalt` als reine Nutzlast, dazu `ereignisart`, und die beiden letzten `neu`, `vorher` und `grund` als eigene Felder. Kein Eintrag trägt ein Feld, das §3.2 nicht nennt.
