...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, zweiundzwanzigste Fassung nach einundzwanzig Gutachten**
...
Der Hinweis wird nach §3.8 bei jeder Materialisierung neu gerechnet. Er steht genau dann, wenn **alle drei** Bedingungen gelten: Der Gewinner des Feldes trägt ein `undoOf`, der Zweitplatzierte stammt **von einem anderen Client als der Gewinner** — verglichen wird die `clientId` aus den beiden Ereignis-Ids, nicht die des Originals —, **und die beiden Werte sind verschieden**. Die dritte Bedingung ist die, an der zwei Rücknahmen desselben Ereignisses auseinandergehen (unten); ohne sie erzeugte jede doppelte Rücknahme einen Hinweis, obwohl niemandes Arbeit verdrängt wurde. `fold.ts` unterdrückt bei `ohneVorherWertVerdraengt` seit M0.2 nach demselben Muster über `wertGleich`. Verdrängt später ein viertes Ereignis den Undo vom Gewinnerplatz, fällt er von selbst weg.

**Warum die `clientId` des Gewinners und nicht die des Originals.** Der Hinweis sagt „Deine Rücknahme hat fremde Arbeit verdrängt", und fremd ist sie gegenüber dem, der die Rücknahme geschrieben hat. Die andere Lesart wäre ebenso formulierbar und ergäbe eine andere Hinweismenge; sie wäre aber falsch, sobald A ein eigenes Ereignis zurücknimmt, das B inzwischen überschrieben hat — genau der Fall, den dieser Paragraph beschreibt. Nach U3 kann ein Client ohnehin nur eigene Ereignisse zurücknehmen; das Original stammt also immer vom Schreiber der Rücknahme, und die Bedingung wäre unter der anderen Lesart nie erfüllt.
...
| S12 | `KAPPUNG_MAX` für `unbekannt` und für die reservierten Ids | 50, global je Liste, ohne Zähler | §3.2 | Anzeigebedarf im gemischten Betrieb; die Zahl bestimmt den Zustand und kostet bei Änderung eine `foldVersion` (§3.9) |
...
**T147** Zwei `EebMeldungEmpfangen` mit derselben `meldungId` und gleichem Inhalt ⇒ **kein** `zweiteAnlageVerworfen`; §3.6 gilt, nicht §3.11. Bei abweichendem `rohPayload` und sonst gleichen Feldern ⇒ `inhaltsschluesselWidersprochen` und `ohneVorherWertVerdraengt` an `meldung/<id>/rohPayload`.
