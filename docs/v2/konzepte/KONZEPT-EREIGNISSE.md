...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, siebenundzwanzigste Fassung nach sechsundzwanzig Gutachten**
...
| `zweiteAnlageVerworfen` | `verworfen: EreignisId` · `gilt: EreignisId` · `inhalt: unknown` — die Nutzlast der verworfenen Anlage **und ihr `grund`**, sofern **das Ereignis** einen trug — nicht: sofern die Art einen verlangt. `grund` ist nach §2.4 überall erlaubt und nur bei neun Arten Pflicht; maßgeblich ist wie überall das Vorhandensein am Ereignis, nicht die Pflichtigkeit der Art (§3.6). Dieselbe Begründung: Die Ablage soll zeigen, was jemand geschrieben hat, und dazu gehört das Warum |
...
**Fünf Zustandsteile ohne Entität, und sie werden verschieden behandelt.** `wartend` und `schichtplan` sind erfasst; `unbekannt`, die Einträge in `verworfeneSchluessel` und `archivierungen` nicht — für die Archivierungen leistet `ZWEITE_ARCHIVIERUNG` (§3.8) dasselbe, für die beiden anderen nichts. Der Trennstrich ist, ob dort **gemeldete Fachdaten** liegen: In `wartend` steht eine Stärke, die jemand gemeldet hat, und sie kann nach der Archivierung eingegangen sein; in `unbekannt` steht ein Ereignis, das dieser Client nicht deuten kann, und in `verworfeneSchluessel` eines, das nicht gewirkt hat. **Für `wartend` gilt deshalb: Trägt eine wartende Beobachtung eine HLC größer als die der maßgeblichen Archivierung, entsteht `nachArchivierungEingegangen` mit dem Entitätspfad, unter dem sie wartet** (`einheit/E7`, `dienstposten/D1`), und mit den `feld`-Werten der zu jungen Beobachtungen in `betroffeneFelder` (`staerke`, `schichtplan/<datum>`). Beides ist aus `wartend` unmittelbar ablesbar.

**Der Hinweis kann dabei wandern, und das ist richtig so.** Trifft die fehlende Anlage nach, steht dasselbe Datum an der Entität oder — bei der Schichtplanzelle — unter `schichtplan/<dienstpostenId>`, und der Hinweis steht dort. Er **verschwindet** nicht, er wechselt den Pfad, weil das Datum den Ort wechselt. Die sechsundzwanzigste Fassung behauptete hier mehr („derselbe Pfad, den die Entität tragen würde") und stimmte für die Schichtplanzelle nicht, weil deren Ziel eine andere Wurzel ist (§3.10). Ausgeschlossen ist allein, dass er ganz ausbleibt (T185). Für die beiden anderen führt §8.2 die Lücke als Nicht-Zusicherung.
...
**Bei den beiden Erstwert-Feldern zählt auch `verdraengt`.** Sonst flackerte der Hinweis: Eine Zusammenführung mit HLC 9 nach einer Archivierung mit HLC 5 stünde in `wartend` als zu jung, verlöre aber nach dem Eintreffen der Anlage gegen den Gewinner mit der **kleineren** HLC und verschwände aus dem Hinweis — obwohl sie unverändert im Zustand steht. Geprüft wird deshalb die höchste HLC **aller** gehaltenen Beobachtungen eines Feldes, nicht nur die des Gewinners (T187). Bei einem `Feld<T>` ist das der Gewinner, bei einem `Erstwert<T>` der jüngste verdrängte Eintrag.
...
| Zweite Anlage: kleinste HLC für `angelegtDurch`, Werte in der gewöhnlichen Auswahl | §3.11 | T1, T90, T100, T143, T186 |
