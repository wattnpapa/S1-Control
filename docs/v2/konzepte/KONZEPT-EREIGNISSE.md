...
7. **`angelegtMitNutzlast` und `angelegtMitGrund`** sind genau **eine** Nutzlast und **ein** Freitext je Entität und wächst damit mit der Zahl der Entitäten, nicht mit der Zahl der Ereignisse. **Nicht** dieselbe Klasse wie die Felder selbst: Die Nutzlast einer `EinheitAufgeteilt` trägt `uebernommeneFahrzeuge` und `uebernommenePersonen` und ist bei einem Löschzug mit vierzig Personen deutlich größer als die Summe der Felder, die sie setzt. Die Schranke hält, der Posten ist aber in B4 getrennt zu messen.
...
Damit das geht, muss **jedes Eingangsdatum eines Hinweises im Zustand stehen**. Das ist der Grund für zehn Angaben, die sonst niemand bräuchte: `Beobachtung.wanduhr`, `.fachlicheZeit` und `.zeitklasse` (für `meldezeitUnplausibel`), `Beobachtung.undoOf` (für `undoTrifftFremdenStand`), `angelegtDurch`, `angelegtMitNutzlast` und `angelegtMitGrund` je Entität, `zurueckgenommenDurch[].hlc` (für `nachArchivierungEingegangen` an einer Rücknahme, §7.3) sowie `verworfeneAnlagen` je Entität und `verworfeneSchluessel` global (für `zweiteAnlageVerworfen`, `reservierteIdVerworfen`, `inhaltsschluesselWidersprochen`). M0.2 hält diese Angaben im Akkumulator statt im Zustand; das ist die Änderung, die `foldVersion 2` unter anderem trägt.
...
**Fünf Zustandsteile ohne Entität, und sie werden verschieden behandelt.** `wartend` und `schichtplan` sind erfasst; `unbekannt` und die Einträge in `verworfeneSchluessel` nicht. **`archivierungen` ist teilweise erfasst:** Eine zweite Archivierung trägt `ZWEITE_ARCHIVIERUNG` (§3.8); eine **Rücknahme** trägt `nachArchivierungEingegangen`, wenn **drei** Bedingungen zugleich gelten:
...
**T192 (unwirksame Rücknahme):** Rücknahme mit unpassender `erwarteteHlc`, eigene HLC 20, bei maßgeblicher Archivierung HLC 5 ⇒ **nur** `wirkungslosGegenTerminalzustand` mit `RUECKNAHME_OHNE_PASSENDE_ARCHIVIERUNG`, **kein** `nachArchivierungEingegangen`; sonst trüge eine folgenlose Handlung zwei Hinweise an demselben Feldpfad.
**T190 (`inhalt` ist überall die reine Nutzlast):** Je ein verworfener Eintrag aus einer Anlage (`EebMeldungEmpfangen`), aus einem ändernden Ereignis auf eine reservierte Id (`AbschnittTypGeaendert(AUFFANG)` mit `grund`) und aus einem `KorrekturVon` ⇒ alle drei tragen `inhalt` als reine Nutzlast, dazu `ereignisart`, und die beiden letzten `neu` und `grund` als eigene Felder, dazu `vorher` nur dort, wo das Ereignis eines trug — bei einem Systemabschnitt, dessen Feld nie durch ein Ereignis gesetzt wurde, fehlt es (§2.2). Kein Eintrag trägt ein Feld, das §3.2 nicht nennt.
