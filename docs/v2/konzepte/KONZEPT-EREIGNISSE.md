...
**T190 (`inhalt` ist überall die reine Nutzlast):** Je ein verworfener Eintrag aus einer Anlage (`EebMeldungEmpfangen`), aus einem ändernden Ereignis auf eine reservierte Id (`AbschnittTypGeaendert(AUFFANG)` mit `grund`) und aus einem `KorrekturVon` ⇒ alle drei tragen `inhalt` als reine Nutzlast, dazu `ereignisart`, und die beiden letzten `neu` und `grund` als eigene Felder, dazu `vorher` nur dort, wo das Ereignis eines trug. Ein Systemabschnitt ist dabei **kein** Sonderfall: §5.3.4 setzt `AUFFANG.typ`, der Bediener sieht den Wert und trägt ihn nach Auflage 6 als `vorher` ein. Kein Eintrag trägt ein Feld, das §3.2 nicht nennt.
