...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, fünfzehnte Fassung nach vierzehn Gutachten**
...
**Warum hier ausnahmsweise auch der Gewinner mitgeführt wird.** Sonst hinge der Inhalt des Hinweises daran, welches Ereignis zuerst eintraf, und die Reihenfolgeabhängigkeit reichte über die Felder hinaus bis in die Hinweismenge. Das Argument aus §3.11 — den Inhalt des Gewinners kann der Zustand nicht halten — gilt für **jede gefaltete Id**; hier geht es allein um die kollidierenden, und deren Zahl ist im Normalbetrieb null (§8.2 führt sie unter den vier Zustandsteilen ohne harte Schranke).
...
| `abschnittAufgeloest` | `aufgeloesterAbschnittId: Id` · `zielAbschnittId: Id` |
...
**Warum `unknown` an vier Stellen, und was jeweils darin liegt.** `gesehen` und `verdraengterWert` tragen den Wert **eines Feldes** — ein Stärke-Tripel, einen Text, eine Struktur —, in der Form, in der er im Feld steht. `inhalt` und die Einträge von `inhalte` tragen dagegen **allein die `nutzlast`** des jeweiligen Ereignisses, nichts sonst: nicht `typ` (der steht in der Art des Hinweises beziehungsweise ist bei einer Anlage durch die Entität bestimmt), nicht `hlc` (sie steht bei `VerworfeneAnlage` als eigenes Feld daneben), nicht `vorher` und `neu` (eine Anlage hat beide nicht). Ohne diesen Satz schrieben zwei Umsetzungen zwei verschiedene Objekte an dieselbe Stelle, und beide läsen §3.11 richtig. Einen engeren Typ als `unknown` gibt es nicht, ohne je Feldpfad einen eigenen Hinweis zu bauen. Prüffall T166.
...
> | `EinheitAufgeteilt` | `wirksameStaerkeRechnerisch(quelle)` **plus der eigenen** `abgeteilteStaerke`, **sofern dieser Abzug wirksam ist** (§5.4.2a) — der Stand, den die Quelle ohne diese eine Aufteilung hätte |
>
...
**B6 — Der rote Ausgang bei doppelt vergebener Ereignis-Id.** §3.6 lässt die Auswahl an der Eintreffreihenfolge hängen; zwei Clients haben dann denselben Versionsvektor und verschiedene `zustandsHash`, und §7.6 der Speicherschicht kennt dafür nur „Fehler“. Zugleich verlangt die M0-Zeile in 03-MEILENSTEINE das geklonte Profil als Pflicht-Fehlerinjektion und misst an der Hash-Konvergenz nach jeder Ruhephase. **Vorschlag:** entweder §7.6 um eine vierte Zeile ergänzen („Versionsvektoren gleich, Hashes verschieden, und beide Zustände tragen `ereignisIdDoppeltVergeben` mit derselben Id ⇒ Protokollbruch, nicht Fold-Fehler"), oder das Abbruchkriterium von M0.4 um denselben Vorbehalt. Beides berührt ein freigegebenes Dokument und gehört Johannes; **es blockiert M1.3 nicht**, wohl aber die Auswertung von M0.4.
...
| Idempotenz über die Ereignis-Id, HLC im Inhaltsvergleich | §3.6 | T81, T163 |
| Idempotenz über den Inhaltsschlüssel | §3.6 | T52, T53, T54, T142 |
| Idempotenz über den Fachvorgang | §3.6 | T25, T28 |
| Unbekannte Art, Version, Feld, Nutzlast, Wert | §3.7 | T91, T92, T93, T94, T16, T160, T161 |
...
**T170 (Hinweis bei doppelter Id ist permutationsstabil):** Zwei Ereignisse derselben Id, in beiden Reihenfolgen gefaltet ⇒ **derselbe** `ereignisIdDoppeltVergeben` mit beiden Nutzlasten in derselben Ordnung; die gesetzten Felder unterscheiden sich, der Hinweis nicht.
