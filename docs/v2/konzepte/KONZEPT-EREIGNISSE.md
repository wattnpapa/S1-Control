...
Stand: 2026-09-09 · Paket M1.2, Bauvorlage für M1.3 · Status: **Entwurf, dreiundzwanzigste Fassung nach zweiundzwanzig Gutachten**
...
4. **`wartend`** hält Beobachtungen zu noch nicht angelegten Entitäten, je Feldpfad so viele, wie seine Aufnahmeoperation vorsieht (§3.3). Sein Schlüsselraum ist die Entitäts-Id aus der Nutzlast, also eine beliebige Zeichenkette vom Draht — er wüchse mit der Zahl der Ereignisse, sobald ein Segment fehlt oder in Quarantäne liegt, und §8.2 sagt selbst, dass ein Client mit Quarantäne sie systematisch sammelt. **Deshalb wird auch `wartend` gekappt: höchstens `KAPPUNG_MAX` Entitätspfade, die mit den kleinsten Pfaden in Codepoint-Ordnung**, und `anlageFehlt` entsteht nur zu den gehaltenen. Es ist dieselbe Minimumsauswahl wie bei `unbekannt` und aus demselben Grund (T180).
...
`EinsatzArchiviert` trägt `{ einsatzId, zeitpunkt, snapshotHash }`. Beide Werte stehen im Zustand unter `archivierungen[<eigene id>]` (§3.2) — sonst wäre der „Beleg" nur im Ereignisstrom auffindbar und nach einem Schnappschuss verloren.
...
**Drei Zustandsteile ohne harte Schranke** (§3.2): die `verdraengt`-Listen der beiden Erstwert-Felder wachsen mit wiederholten Vorgängen an derselben Entität; `verworfeneAnlagen` und die Einträge mit `art: "INHALTSSCHLUESSEL"` in `verworfeneSchluessel` mit doppelt vergebenen Entitäts-Ids und fachlichen Schlüsseln — im Normalbetrieb null, im Klon- oder Migrationsfall nicht; `archivierungen` bekommt je Rücknahme auf eine unbekannte Id einen dauerhaften Eintrag. Alle drei sind klein und in der Praxis leer, aber keine ist nachweisbar beschränkt. `unbekannt`, `wartend` und die Einträge mit `art: "RESERVIERTE_ID"` sind seit der zweiundzwanzigsten und dreiundzwanzigsten Fassung gekappt (§3.2).
...
| S12 | `KAPPUNG_MAX` für `unbekannt`, für `wartend` und für die Einträge mit `art: "RESERVIERTE_ID"` | 50 je dieser drei Mengen, ohne Zähler; die Einträge mit `art: "INHALTSSCHLUESSEL"` bleiben ungekappt | §3.2 | Anzeigebedarf im gemischten Betrieb; die Zahl bestimmt den Zustand und kostet bei Änderung eine `foldVersion` (§3.9) |
...
| Der Zustand ist vollständig beschrieben, acht Schranken | §3.2 | T76, T87, T18, T176, T180 |
...
| Maßgebliche Archivierung, Grabstein, Monotonie, Wurzel | §7.2 | T68, T71, T72, T178 |
...
**T179 (Schichtplan ohne Dienstposten):** `SchichtplanEintragGesetzt(D1, Datum, neu)` ohne `DienstpostenAngelegt(D1)` ⇒ `schichtplan` ist leer, die Beobachtung steht in `wartend`, ein `anlageFehlt` an `dienstposten/D1`; trifft die Anlage ein, erscheint der Eintrag unverändert.
**T180 (Kappung von `wartend`):** 200 Beobachtungen zu 200 verschiedenen, nicht angelegten Einheiten ⇒ 50 Entitätspfade in `wartend` und 50 `anlageFehlt`, in jeder Permutation und über jeden Schnitt dieselben 50.
