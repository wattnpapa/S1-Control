...
| `schemaVersion` | Version der **Nutzlast dieser Ereignisart**, nicht des Rahmens; sie beginnt je Art bei `1` und steigt unabhängig von den anderen. Die Speicherschicht reicht das Feld nur durch; die Upcaster-Kette steht in `KONZEPT-EREIGNISSE.md` §4. Der Rahmen selbst ist über `manifest.json` versioniert (§7.1). *Berichtigt am 2026-09-10 nach Befund B1 des Ereigniskonzepts; die frühere Fassung las das Feld als Rahmenversion und widersprach damit der DoD von M1.2 und ZDM §4.1.* |
...
| `grund` | optional; Freitext, der ins Einsatztagebuch geht. Ein Rahmenfeld und **kein** Bestandteil der Nutzlast: `KONZEPT-EREIGNISSE.md` §3.6 legt ihn bei einem verworfenen Ereignis **neben** `nutzlast` ab, und diese Trennung setzt voraus, dass die Speicherschicht ihn als eigenes Feld führt. Bei neun Ereignisarten ist er Pflicht (§2.4 dort), sonst frei. *Ergänzt am 2026-09-10 nach Befund B6 des Ereigniskonzepts; die Aufzählung nannte ihn nicht, obwohl `ereignis.ts` ihn seit M0.2 führt.* |
...
**Der Undo-Stapel steht nicht hier — er ist abgeleitet.** Auflage 11 verlangt einen Undo-Stapel je Client. Für die Speicherschicht stellt sich dabei nur die Frage, wo er liegt und ob er einen Neustart übersteht. Antwort: Er liegt nirgends. Er wird beim Öffnen des Einsatzes aus dem lokalen Spiegel berechnet — die eigenen Ereignisse in HLC-Ordnung, abzüglich derer, zu denen bereits **ein** Ereignis mit passendem `undoOf` vorliegt — gleichgültig von welchem Client —, begrenzt auf die jüngsten N. *Berichtigt am 2026-09-10 nach Befund B2 des Ereigniskonzepts: Die frühere Fassung verlangte ein **eigenes** Ereignis. Hat ein anderer Client meine Handlung bereits zurückgenommen, ist der alte Stand wiederhergestellt; nähme ich sie erneut zurück, setzte mein Undo denselben Wert ein zweites Mal, mit einer HLC über allem, was zwischenzeitlich geschrieben wurde — ich verwürfe fremde Arbeit, ohne es zu beabsichtigen. Begründung in `KONZEPT-EREIGNISSE.md` §6 U3.* Damit übersteht er jeden Neustart, ohne dass eine eigene Datei ihn tragen müsste, und er kann nicht von den Ereignissen abweichen. Eine gespeicherte Liste könnte das: Sie wäre ein Zustand, der sich nicht allein aus den Ereignissen ergibt, und liefe §1.3 Satz 3 zuwider. N und die Frage, welche Ereignisarten überhaupt rücknehmbar sind, gehören ins Ereigniskonzept.
...
2. Dessen erste Zeile ist ein Ereignis vom Typ `SegmentErsetzt`. Ihre Nutzlast nennt das ersetzte Segment und den Offset, ab dem der Ersatz gilt — und, wenn das ersetzte Segment eine **andere** Kennung trägt, deren Präfix.
   ...
   **Der Ersatz gilt ab der letzten für einen Leser auswertbaren Zeile, nicht ab der ersten abweichenden.** Beide Stellen fallen auseinander, sobald der Schreiber die Abweichung erst nach seinem letzten Übertragungsstand bemerkt: Die Zeilen zwischen der Beschädigungsstelle und diesem Stand stehen physisch auf dem Share, sind hinter der Quarantäne (§8.2 Punkt 7, §2.3) aber für keinen Leser mehr erreichbar. Nähme der Ersatz sie nicht mit, wären sie für alle fort — auch für den Exportweg aus §8.6.1 Regel 4. Maßgeblich ist deshalb das Ende der letzten Zeile, die ein Leser in der Share-Datei noch auswerten kann; liegt die gemeldete Abweichung davor, gilt die frühere der beiden Stellen. Der Schreiber hat alle betroffenen Zeilen lokal (§1.3 Satz 2), die Angabe ist also immer verfügbar.
   ...
   Der Preis ist benannt und in Kauf genommen: Einzelne Zeilen stehen danach zweimal auf dem Share — einmal unlesbar hinter der Beschädigung, einmal lesbar im Ersatz. Der Fold entdoppelt über die Ereignis-Identität („Was ‚gleicher Inhalt' heißt"), ein Leser sieht sie einmal.
...
5. Das beschädigte Segment bekommt keine Abschlusszeile mehr. Es wird nicht mehr beschrieben.
   ...
   **Eine spätere Beschädigung *vor* der Übernahmestelle löst eine erneute Reparatur aus.** „Es wird nicht mehr beschrieben" heißt: in diese Datei schreibt der Schreiber nicht mehr, und ihre liegen gebliebenen Bytes werden nicht noch einmal ersetzt. Es heißt nicht, dass ein neuer Schaden an derselben Datei hinzunehmen wäre. Trifft eine Beschädigung sie an einer Stelle **unterhalb** der Übernahmestelle, so nimmt sie den Lesern Zeilen, die kein Ersatzsegment je wiederholt hat — sie standen bis dahin lesbar da. Dafür entsteht ein weiteres Ersatzsegment, ab der neuen Lesbarkeitsgrenze. Eine Beschädigung ab der Übernahmestelle ist dagegen der bekannte, vorgesehene Endzustand und löst nichts aus. Das Verfahren endet, weil jede weitere Reparatur an einem kleineren Offset ansetzt.
...
2. Der letzte Takt-A-Durchlauf hat für keine dort geführte Datei einen **Fortschritt** gebracht.
3. Der letzte Takt-B-Durchlauf hat keine neue Datei und kein angekündigtes Nachfolgesegment gefunden **und** für keine in Takt B geführte Datei — verfallene, vorläufig quarantänisierte, angekündigte — einen Fortschritt gebracht. Ohne diesen Zusatz wäre die Bedingung für genau die Dateien unbestimmt, die nach §6.2 aus Takt A herausgefallen sind: Sie sind weder abgeschlossen noch in Quarantäne und würden in Takt A gar nicht mehr gelesen.
4. Es ist kein Bedienschritt offen, der noch ein Ereignis erzeugen wird.

**Fortschritt heißt: das gesehene Dateiende ist vorgerückt — nicht: es kamen Bytes.** Eine frühere Fassung verlangte in Bedingung 2 und 3 „0 Bytes geliefert". Das ist nicht messbar: Eine Datei in vorläufiger Quarantäne wird nach §8.1 in **jedem** Takt-B-Durchlauf erneut geprüft und liefert dabei jedes Mal dieselben unvollständigen Bytes. Wörtlich gelesen wäre die Ruhephase damit für den Rest der Lage unerreichbar, sobald irgendwo eine Zeile steht, die niemand mehr vervollständigt — der Fall, den §8.1 ausdrücklich kennt („der Schreiber ist mitten in einer Zeile endgültig ausgefallen"). Ohne Ruhephase gäbe es keinen Konvergenzvergleich und damit kein Abbruchkriterium nach Auflage 18. Maßgeblich ist deshalb, ob die Auswertung vorangekommen ist, also ob sich das gesehene Dateiende verschoben hat. Dieselbe Unterscheidung trifft §6.2 für den Verfall bereits. Ein Durchlauf, der Bytes holt, sie aber nicht übernehmen kann — eine lokale Schreibstörung (§8.8), ein noch unbestimmter Kettenanker —, hat nichts bewegt; der spätere Durchlauf, der es nachholt, sehr wohl.

**Und zwar für `2 + ⌈Cache / Takt⌉` aufeinanderfolgende Durchläufe je Takt.** Die zwei sind nötig, weil ein einzelner leerer Takt A auch dann entsteht, wenn ein anderer Client gerade zwischen zwei Zeilen steht. Das deckt aber nur den Fall ab, in dem der Leser die Wahrheit sieht. Die Caches aus §6.6 halten eine Falschauskunft über ihre Lebensdauer aufrecht: Der Verzeichnis-Cache 10 Sekunden, der `FileNotFound`-Cache 5. Bei einem Takt B von 4 Sekunden (§6.2) werden zwei aufeinanderfolgende Durchläufe aus **derselben** zwischengespeicherten Auskunft bedient; zwei leere Durchläufe belegen dann nicht, dass nichts da ist, sondern nur, dass der Cache noch dieselbe Antwort gibt. Die Ruhephase gälte als erreicht, während eine Datei unentdeckt auf dem Share liegt, und der Konvergenzvergleich fiele danach in den dritten Ausgang, ohne dass irgendetwas kaputt wäre. `Cache` ist die längste Lebensdauer, die im jeweiligen Takt zur Geltung kommt; ohne Caches ergibt die Formel genau die zwei Durchläufe, die hier zuvor standen.

In der Simulation (M0.4) sind diese vier Größen unmittelbar ablesbar; im Feldlauf (M0.5, M2.4) meldet sie `s1 akte pruefe` je Client.
...
**Ersetzte Segmente stehen nicht im Versionsvektor.** Nach einer Reparatur nach §4.6 führt jeder Leser seinen Spiegel des beschädigten Segments bis zur Quarantänestelle — nach §5.5 „ihr geprüftes Präfix" —, während der Schreiber es lokal vollständig hat. Der Offset dieser einen Datei deckt sich damit **nie wieder**, und der Vergleich fiele dauerhaft in die dritte Zeile der Tabelle, obwohl Ereignismenge und Zustand übereinstimmen. Ein Einsatz, in dem einmal repariert wurde, könnte den Konvergenznachweis nie wieder führen — also gerade in den Läufen nicht, die §4.6 prüfen. Das ersetzte Segment fällt deshalb aus dem Vektor heraus, so wie Bedingung 1 der Ruhephase es schon aus der Ruhephase herausnimmt. Verloren geht dabei nichts: Die Ereignisse ab der Übernahmestelle stehen im Ersatzsegment und gehen dort in den Vektor ein, die davor liegenden sind bei allen Clients gleich gefaltet.

Welche Datei ersetzt ist, steht in der ersten Zeile des Ersatzsegments (§4.6 Schritt 2). Damit weiß es **jeder** Client, dessen Spiegel das Ersatzsegment enthält, nicht nur der Schreiber — und nur so fällt die Datei bei allen zugleich heraus. Fiele sie nur beim Schreiber weg, wären die Vektoren wieder über verschiedene Dateimengen gebildet und nach dem Zusatz oben erneut verschieden. Der Vergleich wird durch die Herausnahme nicht milder, sondern **schärfer**: Was zuvor als „nicht vergleichbar" durchging, muss jetzt im `zustandsHash` bestehen.
...
4. **Wiederherstellungsweg.** Der einzige Weg zurück führt über den **Schreiber** der beschädigten Datei. Er hat den Inhalt lokal und stellt die Abweichung zwischen lokaler und Share-Datei beim nächsten Spiegelungslauf fest (§5.4.3). Entscheidend ist, dass er dabei in **Ausgang B** fällt und nicht in Ausgang C: Wäre jede Abweichung ein Fremdschreiberfall, verböte ihm §4.5 das Weiterschreiben unter dieser Kennung, und der einzige, der reparieren könnte, dürfte es nicht — bei gleichzeitig falscher Ursachenmeldung an den Bediener. Weil Ausgang B eine Beschädigung ohne fremde Schreibspur ist, schreibt er den fehlenden Teil als **Ersatzsegment** nach §4.6 neu.
   ...
   **Wie weit die Zusage danach wieder gilt.** Sie gilt für die Ereignisse des ersetzten Segments — nicht darüber hinaus. Wird die Beschädigung erst entdeckt, nachdem der Schreiber das Segment längst verlassen hat, heilt das Ersatzsegment nur dieses eine. Die inzwischen geschriebenen Folgesegmente bleiben für den betroffenen Leser unlesbar: Ihr Kettenanker ist nach §2.3 die letzte Zeile des Vorgängers, und die liegt hinter der Quarantänestelle. Er kann sie nicht prüfen, ohne Unbestätigtes als bestätigt zu führen — was §8.2 verbietet. Für ihn bleibt die Zusage nach §8.2 Punkt 7 ausgesetzt, bis er den Einsatz mit einem gesunden Spiegel neu aufsetzt (Exportweg unten). Das ist ein Bedienschritt, und er ist gewollt: Die Alternative wäre, entweder alle Folgesegmente unbeschränkt zu wiederholen oder eine ungeprüfte Kette in den Fold zu lassen. Beides gibt eine Zusage auf, die dieses Verfahren trägt.
...
| 10 · Zyklusregel; relative Stärkeänderung; Eingangsregel für aufgelöste Abschnitte | — | **Fachliche Fold-Regeln, gehören nach `KONZEPT-EREIGNISSE.md` (M1.2).** Für die Speicherschicht ohne Anforderung; hier bewusst nicht dupliziert |
