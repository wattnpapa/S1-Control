...
*Zu M0.2, geändert am 2026-09-08 (Johannes).* Die ursprüngliche Fassung verlangte „Property-Tests P1 bis P6 grün". P4 (Summenerhaltung) ist eine Aussage über die relative Reduktion der Quellstärke bei `EinheitAufgeteilt` und die additive Zielstärke bei `EinheitZusammengefuehrt`; P6 (monotone Zustandsmaschine) hängt an der Entität `Anforderung` mit `AnforderungAngelegt`, `AbloesungZugesagt`, `AnforderungErledigt` und `AnforderungStorniert` (Zieldatenmodell §4.2 und §4.4). Keine dieser Ereignisarten gehört zu den fünf, die M0.2 ausdrücklich abgrenzt, und unter diesen fünf gibt es überhaupt keine monotone Zustandsmaschine, auf die P6 sich beziehen ließe. Ein Ersatztest unter demselben Namen wäre keine Erfüllung, sondern genau das Schein-Grün, das Auflage 18 für P1 verbietet. Beide Kriterien wandern deshalb nach M1.3, wo der vollständige Ereigniskatalog vorliegt; in `packages/domaene/src/eigenschaften.test.ts` stehen sie bis dahin als übersprungene Tests mit dieser Begründung, damit die Lücke sichtbar bleibt statt zu fehlen.
...
| M1.3 | `@s1/domaene`: Zielmodell (Einsatz, Abschnitt, EinsatzEinheit, Meldung, Fahrzeug, Person, Auftrag, Anforderung, Dienstposten, ETB) mit allen Feldern aus Zieldatenmodell §3; Fold für alle Ereignistypen; Kennzahlen (Summen je Bereich, Organisation, Status, Schicht; Gesamtstärke ohne Anmarsch) mit den Excel-Formeln | Property-Tests grün, **einschließlich P4 (Summenerhaltung) und P6 (monotone Zustandsmaschine)**; Kennzahlen gegen von Hand gerechnete Werte der Referenzlage |
