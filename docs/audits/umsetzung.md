...
Die Audits wurden gegen die Anwendung erhoben, die auf `main` steht und hier
unter `legacy-v1/` liegt. Die Haken unten beziehen sich auf die Umsetzung in
genau dieser Fassung (Branch `claude/thw-skills-audit-hixyfh`). Für den neuen
Baum unter `apps/` und `packages/` wird jeder Punkt eigens geprüft; der Stand
dazu steht in `umsetzung-v2.md`.
...
- [x] Keine Suche, Sortierung, Statusfilter in den Gesamtlisten (command P1-10)
