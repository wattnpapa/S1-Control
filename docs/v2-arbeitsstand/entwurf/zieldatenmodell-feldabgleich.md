...
5. **~~Nach `EinsatzArchiviert` werden neue Ereignisse nicht mehr gefaltet~~** — *gestrichen am 2026-09-10 nach Befund B3.* Die Regel widersprach `KONZEPT-SPEICHER.md` §5.7, und `KONZEPT-EREIGNISSE.md` §7.3 hat für die Speicherfassung entschieden: **Das Ereignis wird angenommen, gefaltet und wirkt.** Verworfen wird nichts; sichtbar gemacht wird es über `nachArchivierungEingegangen` je Entität. Ein Fold, der nach der Barriere aufhörte, verlöre gemeldete Kräfte, ohne dass es jemand sähe — und die Barriere hinge davon ab, wann welcher Client sie sieht.
